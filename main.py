...
if st.session_state.df is not None:
    ...
    selected_types = st.sidebar.multiselect("Select Transaction Type", types, default=types)

    # Amount range filter
    min_amount, max_amount = float(filtered_df['Amount'].min()), float(filtered_df['Amount'].max())
    selected_amount = st.sidebar.slider("Select Amount Range", min_amount, max_amount, (min_amount, max_amount))
    ...
    with tab1:
        ...
        weekly = filtered_df.groupby(filtered_df['Datetime'].dt.to_period('W'))['Amount'].sum().to_timestamp()
        monthly = filtered_df.groupby(filtered_df['Datetime'].dt.to_period('M'))['Amount'].sum().to_timestamp()
        st.line_chart(weekly.rename("Weekly Spend"))
        st.line_chart(monthly.rename("Monthly Spend"))

        st.subheader("Cumulative Cash Flow")
        filtered_df['DebitAmount'] = filtered_df.apply(lambda x: x['Amount'] if x['Type']=='Debited' else 0, axis=1)
        filtered_df['CreditAmount'] = filtered_df.apply(lambda x: x['Amount'] if x['Type']=='Credited' else 0, axis=1)
    ...
    with tab2:
        ...
        fig = px.bar(top_merchants, x=top_merchants.values, y=top_merchants.index, orientation='h',
        ...
        def categorize(desc):
            desc = str(desc).lower()
            if any(x in desc for x in ['zomato','restaurant','cafe','bar']): return 'Food'
            if any(x in desc for x in ['uber','ola','taxi','travel']): return 'Travel'
            if any(x in desc for x in ['amazon','flipkart','shopping']): return 'Shopping'
            if any(x in desc for x in ['bill','electricity','water','utility']): return 'Utilities'
            return 'Other'
    ...
    with tab3:
        ...
        pivot = filtered_df.pivot_table(values='Amount', index='DayOfWeek', columns='Hour', aggfunc='sum', fill_value=0)
